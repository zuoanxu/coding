...
public class OrderService {
    ...
    public Order createOrder(CreateOrderRequest request) {
        ...
        try {
            ...
            SendResult sendResult = rocketMQTemplate.syncSend("order-delay-topic:cancel", msg, 5000, 16);

            // 关键：只有 SEND_OK 才算发送成功，否则抛异常，让整个下单回滚
            if (sendResult == null || sendResult.getSendStatus() != SendStatus.SEND_OK) {
                throw new BusinessException("发送延时取消消息失败，请重试");
            }
